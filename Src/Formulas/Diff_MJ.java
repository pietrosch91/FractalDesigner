...
class Diff_Mandelbrot extends Formula{
	double px,py;
	double X,Y;
	double bailout;
	double wlast;
	
	public Diff_Mandelbrot(){
		NPars=3;
	}
	...
	public void Evolve(){
		...
		if(Niters>0){
			zrlast-=wlast*zr[Niters-1];
			zilast-=wlast*zi[Niters-1];
		}
	//	double tx=zr*zr-zi*zi+X;
	//	double ty=2*zr*zi+Y;
		zr[Niters+1]=zrlast*zrlast-zilast*zilast+X;//-zr[Niters];
        zi[Niters+1]=sig*(2*zrlast*zilast+Y);//-zi[Niters];
        sig*=falt;    
        Niters++;		
	}
...
class Diff_Julia extends Formula{
	double px,py;
	double wlast;
	double X,Y;
	double bailout;
...
class Diff_General_Mandelbrot extends Formula{
	double px,py;
	double X,Y;
	double []cr;
	double []ci;
	...
	public Diff_General_Mandelbrot(){
		NPars=6;
		cr=new double[4];
		ci=new double[4];
		rho=new double[4];
...
class Diff_General_Julia extends Formula{
	double px,py;
	double X,Y;
	double []cr;
	double []ci;
