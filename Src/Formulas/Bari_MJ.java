...
class Bari_Mandelbrot extends Formula{
	double px,py;
	//double []Zr;
	//double []Zi;
	double X,Y;
	...
	//	double ty=2*zr*zi+Y;
		double zrn=zrlast*zrlast-zilast*zilast+X;
        double zin=sig*(2*zrlast*zilast+Y);
        double zrf=zrn;
        double zif=zin;
        for(int i=0;i<naverage-1;i++){
			zrf+=zr[GetIndex(Niters-i)];
			zif+=zi[GetIndex(Niters-i)];
		}
		zr[Niters+1]=zrf/RedoxFactor;
		zi[Niters+1]=zif/RedoxFactor;
        sig*=falt;    
        Niters++;		
	}
	...
	public double FinalSmoothingNorm(){
		...
		return Niters-1+(bailout-GetRho(Niters-1))/(GetRho(Niters)-GetRho(Niters-1));
	} 
...
class Bari_Julia extends Formula{
	double px,py;
	int naverage;
	double RedoxFactor;
	double X,Y;
