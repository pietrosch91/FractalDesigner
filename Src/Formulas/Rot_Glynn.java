...
class Rot_Glynn_Julia extends Formula{
	double px,py;
	double expo;	
	double ARA;
	double X,Y;
	...
	//	double tx=zr*zr-zi*zi+X;
	//	double ty=2*zr*zi+Y;
		zrlast=rhonew*cos(phinew)+X;
        zilast=sig*(rhonew*sin(phinew)+Y);
        //backrotation
        zr[Niters+1]=zrlast*RotR+zilast*RotI;
        zi[Niters+1]=zilast*RotR-zrlast*RotI;        
        sig*=falt;    
        Niters++;		
	}
