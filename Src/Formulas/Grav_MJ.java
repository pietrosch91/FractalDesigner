...
class Grav_Mandelbrot extends Formula{
	double px,py;
	double Cx,Cy;
	double cfac;
	double X,Y;
	...
	//	double ty=2*zr*zi+Y;
		zr[Niters+1]=cfac*(zrlast*zrlast-zilast*zilast+X-Cx)+Cx;
        zi[Niters+1]=cfac*(sig*(2*zrlast*zilast+Y)-Cy)+Cy;
        sig*=falt;    
        Niters++;		
	}
...
class Grav_Julia extends Formula{
	double px,py;
	double X,Y;
	double Cx,Cy;
